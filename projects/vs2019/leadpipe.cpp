...
void CLeadpipe::Spawn()
{
	Precache();
	m_iId = WEAPON_LEADPIPE;
...
void CLeadpipe::Precache()
{
	...
	m_usLeadpipe = PRECACHE_EVENT(1, "events/crowbar.sc");
}

bool CLeadpipe::GetItemInfo(ItemInfo* p)
{
	...
	p->iSlot = 0;
	p->iPosition = 0;
	p->iId = WEAPON_LEADPIPE;
	p->iWeight = LEADPIPE_WEIGHT;
	return true;
}
...
{
	return DefaultDeploy("models/v_crowbar.mdl", "models/p_crowbar.mdl", LEADPIPE_DRAW, "crowbar");
}
...
void CLeadpipe::PrimaryAttack()
{
	if (!Swing(true))
	{
		SetThink(&CLeadpipe::SwingAgain);
		pev->nextthink = gpGlobals->time + 0.1;
	}
}


void CLeadpipe::Smack()
{
	DecalGunshot(&m_trHit, BULLET_PLAYER_CROWBAR);
}


void CLeadpipe::SwingAgain()
{
	Swing(false);
}


bool CLeadpipe::Swing(bool fFirst)
{
	bool fDidHit = false;
	...
	if (fFirst)
	{
		PLAYBACK_EVENT_FULL(FEV_NOTHOST, m_pPlayer->edict(), m_usLeadpipe,
